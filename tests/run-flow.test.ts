...
describe('run flow', () => {
  ...
  it('a wipe loses the round: the gauge goes back to the round start, gold kept, team healed', () => {
    ...
    // Mid-round, one round already done; a wild Pokémon is the next card.
    useGame.setState({
      save: {
        ...s,
        gold: 42,
    ...
    expect(progressOf(after, a1.id)).toMatchObject({ roundsDone: 1, deck: [], drawn: [] }) // next encounter: a new round
