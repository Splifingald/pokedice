...
export function bundleToRows(b: BundleRaw): TableRows {
  return {
    type_chart: b.typeChart.map((x) => ({ attacking: x.attacking, defending: x.defending, multiplier: x.multiplier })),
    ...
    combo_upgrades: b.upgrades.combos.map((x) => ({ combo_key: x.comboKey, level: x.level, bonus: x.bonus, cost: x.cost })),
    die_upgrades: b.upgrades.dice.map((x) => ({ die_type: x.dieType, level: x.level, bonus: x.bonus, cost: x.cost })),
    items: b.items.map((x) => ({
      key: x.key,
      name: x.name,
      description: x.description,
