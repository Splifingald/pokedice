...
function SellRow({ it, owned }: { it: ItemDef; owned: number }) {
  ...
  const choices = [...new Set([1, 5, owned].filter((q) => q <= owned))].map((q) => ({ q, text: q === owned && q > 1 ? `All ×${q}` : `×${q}` }))
      ...
      <div className="flex items-center gap-3">
        ...
        {unit > 0 ? (
          <PixelButton
            variant={confirming ? 'success' : 'primary'}
            className="min-w-[104px] shrink-0 px-2"
            aria-label={`Sell ${n} ${it.name} for ${money(unit * n)}`}
            onClick={() => {
              if (!confirming) return setConfirming(true)
          ...
          <span className="shrink-0 text-lg text-muted">Can't sell</span>
        )}
      </div>
      {open && (
        <div id={detailsId} className="mt-2 flex flex-wrap items-center gap-3 border-t-2 border-dashed border-shadow pt-2">
          ...
          {unit > 0 && choices.length > 1 && (
            <QtyPicker
              label={`How many ${it.name} to sell`}
              options={choices}
              value={n}
              onPick={(q) => {
                setQty(q)
...
function SellTab() {
  ...
  if (!bag.length) return <p className="copy text-muted">Your bag is empty.</p>
  return (
    <>
      <p className="copy text-muted">
        Your bag. The Mart buys back anything it sells for half its price. Tap an item for details and to sell several;
        the price button asks once to confirm.
      </p>
      {GROUPS.map((g) => {
...
export function ShopScreen() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4">
      ...
      <div className="flex gap-2" role="tablist" aria-label="Buy or sell">
